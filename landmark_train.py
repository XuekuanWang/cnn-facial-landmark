...
tr_data, tr_label = get_one_batch_data(batch_size, 0)
te_data, te_label = get_one_batch_data(batch_size, 1)

# image_size = 32, img_channels = 3, class_num = 10 in cifar10
x = tf.placeholder(tf.float32, shape=[None, image_size, image_size, img_channels])
label = tf.placeholder(tf.float32, shape=[None, point_number])

logits = SE_ResNeXt(x, is_training=True, keep_prob=0.8).model
...
with tf.Session() as sess:
    ...
    for epoch in range(1, total_epochs + 1):
        ...
        for step in range(1, iteration + 1):
            batch_x, batch_y = sess.run([tr_data, tr_label])
            train_feed_dict = {
                x: batch_x,
                label: batch_y,
            }
            # print(batch_x, batch_y)
            global_step_val, lr_val, _, batch_loss = sess.run([global_step, lr,  train, cost], feed_dict=train_feed_dict)
            print(epoch, step, global_step_val, batch_loss, lr_val)
        ...
        saver.save(sess=sess, save_path='./model/ResNeXt.ckpt' + str(epoch))